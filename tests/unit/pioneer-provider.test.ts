...
test("pioneer has open-tier models with supports_on_demand_inference", () => {
  ...
  assert.ok(!ids.some((id) => id.toLowerCase().includes("claude")), "gated Claude models must not be in catalog");
  assert.ok(!ids.some((id) => id.toLowerCase().includes("gpt")), "gated GPT models must not be in catalog");
  assert.ok(!ids.some((id) => id.toLowerCase().includes("gemini")), "gated Gemini models must not be in catalog");
});
...
test("pioneer has hasFree flag and free signup notice", () => {
  ...
  assert.ok(pioneer.notice?.signupUrl?.includes("pioneer.ai"), "signupUrl should point to pioneer.ai");
  assert.ok(pioneer.notice?.apiKeyUrl?.includes("pioneer.ai"), "apiKeyUrl should point to pioneer.ai");
});
